          ...
          <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-4">
            <div className="relative md:col-span-2">
              ...
              <Input
                ...
                className="border-[#D4E8FC] bg-[#F8FBFF] pl-9 text-slate-900 placeholder:text-slate-500 focus-visible:ring-[#1B66B3]/40"
              />
            </div>

            {filters.map((filter) => (
              <Select key={filter.label} value={filter.value} onValueChange={filter.onChange}>
                <SelectTrigger className="border-[#D4E8FC] bg-[#F8FBFF] text-slate-900 focus:ring-[#1B66B3]/40">
                  <SelectValue placeholder={filter.label} />
                </SelectTrigger>
                <SelectContent className="border-[#D4E8FC] bg-[#F8FBFF] text-slate-900">
                  {filter.options.map((option) => (
                    <SelectItem key={option.value} value={option.value} className="focus:bg-[#EDF5FF]">
                      {option.label}
                    </SelectItem>
                  ))}
                </SelectContent>
